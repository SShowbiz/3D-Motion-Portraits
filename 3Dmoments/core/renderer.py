...
class ImgRenderer:
    ...
    def feature_extraction(self, rgba_layers, mask_layers, depth_layers):
        ...
        if self.args.use_depth_for_feature:
            rgba_layers_in = torch.cat([rgba_layers_in, 1.0 / torch.clamp(depth_layers.squeeze(1), min=1.0)], dim=1)
        featmaps = self.model.feature_net(rgba_layers_in)
        return featmaps

    def apply_scale_shift(self, depth, scale, shift):
        disp = 1.0 / torch.clamp(depth, min=1e-3)
    ...
    def render_pcd(self, pts1, pts2, rgbs1, rgbs2, feats1, feats2, mask, side_ids, R=None, t=None, time=0):
        ...
        if self.args.use_depth_for_decoding:
            disp = normalize_0_1(1.0 / torch.clamp(pts_[..., [-1]], min=1e-6))
        ...
        for j in range(num_sides):
            ...
            renderer = create_pcd_renderer(
                self.h,
                self.w,
                self.tgt_intrinsic.squeeze()[:3, :3],
        ...
        w1, w2 = self.compute_weight_for_two_frame_blending(
            time,
            global_out_list[0][..., [-1]],
            global_out_list[-1][..., [-1]],
            global_out_list[0][..., [3]],
        ...
        pred_rgb = self.model.img_decoder(
            global_out_list[0].permute(0, 3, 1, 2), global_out_list[-1].permute(0, 3, 1, 2), time
        )
    ...
    def get_reprojection_mask(self, pts, R, t):
        ...
        mask_renderer = create_pcd_renderer(
            self.h, self.w, self.tgt_intrinsic.squeeze()[:3, :3], radius=1.0 / min(self.h, self.w) * 4.0
        )
    ...
    def render_rgbda_layers_from_one_view(self):
        ...
        rgba_layers, depth_layers, mask_layers = self.inpainter.sequential_inpainting(
            self.src_img1, self.src_depth1, depth_bins
        )
        ...
        pred_img, direct_rgb_out, meta = self.render_pcd(
            all_pts, all_pts, all_rgbas, all_rgbas, all_feats, all_feats, all_masks, all_side_ids, R, t, 0
        )
    ...
    def compute_scene_flow_one_side(
        self,
        coord,
        pose,
        rgb1,
        ...
        pts2_sampled = F.grid_sample(
            pts2, coord1_corsp_normed, align_corners=True, mode="nearest", padding_mode="border"
        )
        ...
        for i in range(num_layers1):
            ...
            flow_inpainted, mask_no_mutual_flow_ = self.masked_diffuse(
                flow_f.permute(0, 3, 1, 2), mask_no_mutual_flow, kernel_size=15, iter=7
            )
            ...
            depth_inpainted_, mask_gau_i_ = self.masked_diffuse(depth2_sampled, mask_gau_i, kernel_size=15, iter=7)
            ...
            pts2_i = unproject_pts_pt(
                self.intrinsic2, coord_inpainted.contiguous().reshape(-1, 2), depth_inpainted.flatten()
    ...
    def render_rgbda_layers_with_scene_flow(self, return_pts=False):
        ...
        rgba_layers1, depth_layers1, mask_layers1 = self.inpainter.sequential_inpainting(
            self.src_img1, self.src_depth1, depth_bins1
        )
        rgba_layers2, depth_layers2, mask_layers2 = self.inpainter.sequential_inpainting(
            self.src_img2, self.src_depth2, depth_bins2
        )
        ...
        (
            ...
            all_masks_1,
            all_optical_flow_1,
        ) = self.compute_scene_flow_one_side(
            coord,
            torch.inverse(self.pose),
            self.src_img1,
            self.src_img2,
            rgba_layers1,
            rgba_layers2,
    ...
    def save_rgbda_layers(self, src_rgb_file, rgba_layers, depth_layers, mask_layers):
        ...
        out_dir = os.path.join(self.args.rootdir, "out", self.args.expname, "vis", "{}-{}".format(scene_id, frame_id))
        ...
        for i, depth_layer in enumerate(depth_layers):
            ...
            disparity = (1.0 / torch.clamp(depth_layer, min=1e-6)) * alpha_layers[i]
