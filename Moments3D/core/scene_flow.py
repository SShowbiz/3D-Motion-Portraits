...
class SceneFlowEstimator:
    ...
    def get_mutual_matches(self, flow_f, flow_b, th=2.0, return_mask=False):
        ...
        grid2 = grid + flow_f
        mask_boundary = (
            (grid2[..., 0] >= 0) * (grid2[..., 0] <= w - 1) * (grid2[..., 1] >= 0) * (grid2[..., 1] <= h - 1)
        )
        grid2_normed = normalize_for_grid_sample(grid2, h, w)
        flow_b_sampled = F.grid_sample(flow_b.permute(0, 3, 1, 2), grid2_normed, align_corners=True).permute(0, 2, 3, 1)
        grid1 = grid2 + flow_b_sampled
        mask_boundary *= (
            (grid1[..., 0] >= 0) * (grid1[..., 0] <= w - 1) * (grid1[..., 1] >= 0) * (grid1[..., 1] <= h - 1)
        )
